...
func SetLatticeInAndOutTag(c *context.InstructionContext) {
	//for _,l := range c.GetLatticeIn(){
	//	if val,ok := LatticeTable.Contain(l.GetKey());ok{
	//		l.SetTag(val.GetTag())
	//	}else{
	//		LatticeTable.Add(l.GetKey(),l)
	//	}
	//}
	//for _,l := range c.GetLatticeIn(){
	//	LatticeTable.GetLattice()
	//	c.GetLatticeOut().LeastUpperBound(l)
	//	log.Debugf("lattice out:%s",c.GetLatticeOut().String())
	//}
}

func SetLatticeInTag(c *context.InstructionContext){
	for _,l := range c.GetLatticeIn(){
		if val,ok := LatticeTable.Contain(l.GetKey());ok{
			l.SetTag(val.GetTag())
		}else{
...
func SetLatticeOutTag(c *context.InstructionContext) {
	for _,l := range c.GetLatticeIn(){
		c.GetLatticeOut().LeastUpperBound(l)
	}
}
...
func CheckSource(c *context.InstructionContext) bool {
	if c.IsCall(){
...
func CheckSink(c *context.InstructionContext) bool {
	if c.IsCall(){
		call := c.GetInstr().(ssa.CallInstruction)
		callcom := call.Common()
		sig,callee,isig := getSignature(callcom)
