...
func (l Lattice) TransferFunction(node ssa.Instruction, ptr *pointer.Result) transferFunction.PlainFF {
	// Handling the different possibilities for a ssautils.Instruction
	//log.Printf("ptr is nil %t", ptr == nil)
	switch nType := node.(type) {
	// Handle all cases which returns only the id
	// *ssautils.MakeClosure returns only the id, becuase it's a ~function~ call which creates a new context
	case *ssa.DebugRef, *ssa.Jump, *ssa.MakeClosure, *ssa.Panic, *ssa.Return:
		return returnID
		// Handle the cases which operates on one ssautils.Value e.g. Type.X and requires a LUP
		// A allocation should set a value to untainted
		// *ssautils.MakeInterface is not listed because there construct a new type based upon another type
	...
	case *ssa.ChangeInterface, *ssa.ChangeType, *ssa.Convert, *ssa.Extract, *ssa.Field, *ssa.FieldAddr, *ssa.Index, *ssa.MakeInterface, *ssa.Next, *ssa.Range, *ssa.Send, *ssa.Slice, *ssa.TypeAssert, *ssa.UnOp:
		...
		// Handle the cases which operates on two ssautils.Values
	...
	case *ssa.Phi:
		...
		// Hande the case with three ssautils.Values
	case *ssa.MapUpdate:
	...
	case *ssa.If:
		// handling the ssautils representation for an if statement
		return returnID
	case *ssa.RunDefers:
