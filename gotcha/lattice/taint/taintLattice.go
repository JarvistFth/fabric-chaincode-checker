...
// Lattice is data structure (map) which maps a ssautils.Value to the abstract value.
// ssautils.Value is an interface -> no pointer needed because it's already a pointer.
...
// getTaintLattice returns the Lattice instance of an LatticeInterface instance
// Support Lattice and LatticePointer
// For other instances of LatticeInterface, the function will return an error.
...
// The function supports all intances of LatticeInterface which returns a lattice when called with getTaintLattice.
func (l Lattice) LeastUpperBound(l2 lattice.Latticer) (lattice.Latticer, error) {
	...
	if len(l) > len(l2taint) {
		smallerL = l2taint.DeepCopy().(Lattice)
		biggerL = l.DeepCopy().(Lattice)
	} else {
		smallerL = l.DeepCopy().(Lattice)
...
RangeLO:
	...
	RangeLI:
		for ssaValI, valI := range smallerL {
			...
			// found a match between two ssautils values -> build the lup of them
			if ssaValO == ssaValI {
...
func (l Lattice) GreatestLowerBound(l2 lattice.Latticer) (lattice.Latticer, error) {
	retMap := NewLattice(len(l))
	// Deep copy of l1 into the return map
	for ssaValThis, valueThis := range l {
	...
	for ssaValL2, valL2 := range l2taint {
		// Iterating over l1 to have the chance to compare the ssautils.Values.
		added = false
...
// lowest abstract value of LatticeTagInterface being Empty
func (l Lattice) LeastElement() (lattice.Latticer, error) {
	retMap := make(Lattice)
	for ssaVal := range l {
...
func (l Lattice) Less(l2 lattice.Latticer) (bool, error) {
	l2taint, err := getTaintLattice(l2)
	if err != nil {
		return false, errors.Wrap(err, "failed less get taint lattice")
	}
	for ssaValL1, valL1 := range l {
		// Iterating over l2 to have the chance to compare the ssautils.Values.
		var visited bool
		visited = false
		for ssaValL2, valL2 := range l2taint {
			// Comparising with Name () - but its propably not unique
...
// Throws an error if l2 is not of type taint.Lattice.
func (l Lattice) Equal(l2 lattice.Latticer) (equal bool, err error) {
	l2taint, err := getTaintLattice(l2)
	if err != nil {
...
func (l Lattice) LessEqual(l2 lattice.Latticer) (lesseq bool, err error) {
	l2taint, err := getTaintLattice(l2)
	if err != nil {
		return false, errors.Wrap(err, "")
	}
	lesseq, err = l.Equal(l2taint)
	if err != nil {
		return false, errors.Wrap(err, "")
	}
	if lesseq {
...
// Throws an error if t2 is not of type taint.Lattice.
func (l Lattice) Greater(l2 lattice.Latticer) (bool, error) {
	l2taint, err := getTaintLattice(l2)
	if err != nil {
		return false, errors.Wrap(err, "")
	}
...
func (l Lattice) GreaterEqual(l2 lattice.Latticer) (greatereq bool, err error) {
	l2taint, err := getTaintLattice(l2)
	if err != nil {
		return false, errors.Wrap(err, "")
	}
	greatereq, err = l.Equal(l2taint)
	if err != nil || greatereq {
		return
	}
	greatereq, err = l.Greater(l2taint)
	return
}

// BottomLattice sets for all elements of the lattice the value to empty.
func (l Lattice) BottomLattice() lattice.Latticer {
	retLattice := make(Lattice)
	for ssaValL1 := range l {
...
func (l Lattice) DeepCopy() lattice.Latticer {
	retLattice := make(Lattice)
	for ssaVal, latVal := range l {
...
func (l Lattice) GetVal(key ssa.Value) lattice.Valuer {
	lVal := l[key]
...
func (l Lattice) SetVal(key ssa.Value, value lattice.Valuer) error {
	val, ok := value.(Value)
	if !ok {
