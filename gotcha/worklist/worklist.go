...
func setLogger() {
	var err error
	logFile, err = os.OpenFile("./debuglogs/"+now+"_Log.txt", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	...
	logStat, err := os.OpenFile("./debuglogs/"+now+"_Log.stat", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	//	defer logStat.Close()
	if err != nil {
...
func flow(c *ContextCallSite) error {
	...
	// The flow function can also handle on not ssautils.Values, but the lattice needs a ssautils.Value
	...
	case ssa.Value, *ssabuilder.Send, *ssa.Store, *ssa.Go, *ssa.Defer:
		...
		// Get the correct value for the flow function (for Send and ssautils.Store the ssautils.Value is within the type)
	...
	case *ssa.Jump, *ssa.Return:
		// ToDo improve - current problem: can't pass n as value for the lattice (is *ssautils.Instruction)
		c.SetOut(c.GetIn())
	}
	...
	case ssa.CallInstruction, *ssa.MakeClosure, *ssa.Call:
		...
		case *ssa.MakeClosure:
			...
			if !ok {
				errors.Errorf("unexpected: call(%s).Fn should be of type *ssautils.Function", call)
			}
